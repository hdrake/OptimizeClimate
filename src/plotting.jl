...
function plot_state(model::ClimateModel; new_figure=true, plot_legends=true)
    ...
    plot(model.domain, f_med(model.controls.remove) * model.economics.remove_cost, color="C0", label=L"$C_{\phi} f(\phi)$ (negative emissions)")
    plot(model.domain, f_med(model.controls.mitigate) * model.economics.mitigate_cost, color="C1", label=L"$C_{\varphi} f(\varphi)$ (emissions reductions)")
    plot(model.domain, f_med(model.controls.adapt) * model.economics.adapt_cost, color="C2", label=L"$C_{\chi} f(\chi)$ (adaptation)")
    plot(model.domain, f_med(model.controls.geoeng) * model.economics.geoeng_cost, color="C3", label=L"$C_{\lambda} f(\lambda)$ (geoengineering)")
...
function plot_ensemble(ensemble::Dict{String, ClimateModel})
    ...
    plot(model.domain, f_med(model.controls.mitigate) * model.economics.mitigate_cost, label=L"$C_{\varphi} f(\varphi)$ (emissions reductions)")
