...
\begin{enumerate}
    ...
    % \item future trajectories of control deployments are optimized from the vantage point of $t = t_{0}$,
    % \item control deployments are integrated forward one policy-making period $\Delta t$,
    % \item model parameters are revised, owing to new information or policy decisions
    % \item future trajectories of control deployments are re-optimized, now from the vantage point of $t = t_{0} + \Delta t$ and with updated model parameters,
    % \item and we return to step 2, repeating the process as desired.
\end{enumerate}
